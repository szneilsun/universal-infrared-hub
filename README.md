...
基于 ESP32-S3 / ESP32-C6、HomeSpan 和 Arduino-IRremote，将美菱空调、Leader 空调
和 Pioneer 机顶盒接入 Apple 家庭 App。当前版本为 `3.1.1`，并支持通过网页学习通用
红外设备、动态创建 HomeKit 子配件。

![ESP32 HomeKit 红外 Hub 海报](./assets/esp32-ir-hub-poster-v2-final.png)

> 让老家电住进 Apple 家庭：支持红外学习、网页配置与 Home App 自动化联动。
...
- 空调开机与关机
- Leader 空调：开关、制冷、16.0～30.0℃（0.5℃步进）与自动、静音、1～5档、强力风
- 制冷、制热模式切换
...
- ESP32-S3：GPIO 41 为红外接收模块信号，GPIO 40 为红外发射驱动输入
- ESP32-C6：GPIO 3 为红外接收模块信号，GPIO 4 为红外发射驱动输入
- 串口波特率：115200

红外 LED 不应由 GPIO 直接驱动。建议使用 S8050、2N2222 或逻辑级 MOSFET：

```text
IR_SEND_PIN ── 1kΩ ── NPN 基极
ESP32 GND ──────── NPN 发射极
...
每次断电后重新上电时，设备都会创建 5 分钟的配置热点：

1. 连接热点 `IR-AC-Setup`。
2. 热点密码为 `iracsetup`。
3. 打开 `http://192.168.4.1`，添加当前环境的家庭 Wi-Fi。
