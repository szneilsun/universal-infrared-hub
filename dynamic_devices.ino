...
String pageHeader() {
  return F("<!doctype html><html lang='zh-CN'><meta name='viewport' content='width=device-width,initial-scale=1,viewport-fit=cover'>"
           ...
           ".eyebrow{margin:0 0 9px;color:var(--sage-dark);font-size:12px;font-weight:700;letter-spacing:.14em}.hero h1{margin:0;font-family:ui-serif,Georgia,'Songti SC',serif;font-size:30px;letter-spacing:.04em;font-weight:600}.hero p{max-width:530px;margin:10px 0 0;color:var(--muted);font-size:14px;line-height:1.65;position:relative;z-index:1}.access{display:inline-block;margin-top:17px!important;padding:10px 12px;border-radius:10px;background:rgba(255,255,255,.62);color:var(--sage-dark)!important;font-size:13px!important;line-height:1.7!important}.access b{font-family:ui-monospace,SFMono-Regular,Menlo,monospace;font-size:12px;word-break:break-all}"
...
void sendOtaPage() {
  ...
  const uint32_t currentSize = ESP.getSketchSize();
  String html = pageHeader();
  html += F("<section><h2>固件升级</h2>"
            "<p class='muted'>请选择为当前芯片编译的应用程序 .bin 固件。写入完成后 Hub 会自动重启。</p>"
            "<p class='muted'><b>OTA 文件限制</b><br>最大文件：");
  html += String(otaCapacity / 1048576.0f, 2) + " MiB（" + String(otaCapacity) + " 字节）";
  html += "<br>当前固件：" + String(currentSize / 1048576.0f, 2) + " MiB（" + String(currentSize) + " 字节）";
  html += F("<br>仅支持与当前芯片匹配的应用固件；请勿上传 bootloader、partitions 或 merged 镜像。</p>"
            ...
            "<button id='ota-submit' type='submit'>上传并升级</button></form>"
            "<p id='file-state' class='muted'>选择文件后会在浏览器中先检查大小。</p>"
            "<p><a href='/'>返回设备管理</a></p>"
            "<script>const max=");
  html += String(otaCapacity);
  html += F(",input=document.getElementById('firmware'),button=document.getElementById('ota-submit'),state=document.getElementById('file-state');"
            "input.addEventListener('change',()=>{const file=input.files[0];if(!file){button.disabled=false;return;}"
            "const size=(file.size/1048576).toFixed(2);if(file.size>max){button.disabled=true;state.textContent='文件 '+size+' MiB，超过本设备 OTA 上限，无法上传。';}"
            "else{button.disabled=false;state.textContent='文件 '+size+' MiB，大小符合限制。';}});</script></section></body></html>");
